...
public:
    ...
    std::optional<std::tuple<double, double>> getThermalProtectionThresholds();
    ...
    bool setFramePeriodMsLimits(std::tuple<uint32_t, uint32_t> minAndMaxMs);
    bool setIntegrationTimeUsLimits(std::tuple<uint16_t, uint16_t> minAndMaxUs);
    bool setLogSettings(std::optional<std::string> globalEnables, std::optional<std::string> categoryLevels);
    bool setMinAmplitudeLimits(std::tuple<uint16_t, uint16_t> minAndMaxAmplitude);
    bool setModelName(const char *modelName);
    bool setModulationFreqKhzLimitsAndStep(std::tuple<uint16_t, uint16_t, uint16_t> minMaxStepKhz);
    ...
    bool setThermalProtectionThresholds(std::tuple<double, double> thresholds);
    bool setVledMinMax(std::tuple<uint16_t, uint16_t> vledLimits);
