...
static void parseArgs(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("binary-output,B", "Output BINARY data")
        ("text-output,t", "Output TEXT")
        ("hex-output,x", "Output HEX")
        ("extracted-mode,e", "EXTRACTED mode access")
        ("raw-mode,r", "storage mode RAW")
        ...
        ("cal-file,P", po::value<std::string>(&calFileName), "Program calibration data from calfile AND EXIT")
    ...
    if(vm.count("M")) 
    {
        ...
        unsigned ipv4GW[4] {0};
        char deviceSerialNum[SERIAL_NUMBER_SIZE] {0};
        char cpuBoardSerialNum[SERIAL_NUMBER_SIZE] {0};
        char modelName[SERIAL_NUMBER_SIZE] {0};
        char testStationData[TEST_STATION_DATA_SIZE] {0};
        int n = sscanf(manufacturingDataString.c_str(), "%u.%u.%u.%u,%u.%u.%u.%u,%u.%u.%u.%u,%255[^,],%255[^,],%255[^,],%2040[^,]",
                        ipv4Address+0, ipv4Address+1, ipv4Address+2, ipv4Address+3,
                        ipv4Mask+0, ipv4Mask+1, ipv4Mask+2, ipv4Mask+3, ipv4GW+0, ipv4GW+1, ipv4GW+2, ipv4GW+3,
                        deviceSerialNum, cpuBoardSerialNum, modelName, testStationData);
        if(16 == n)
        {
