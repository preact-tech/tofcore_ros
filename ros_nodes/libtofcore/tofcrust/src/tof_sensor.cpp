...
std::optional<StorageMetadata_T> Sensor::getStorageMetadata(TofComm::StorageId_e id, TofComm::StorageMode_e mode)
{
    ...
    auto isOk { false };
    auto startProtocol = this->getProtocolVersion();
    if (0 == startProtocol)
    {
        this->setProtocolVersion(1); // cannot use version 0 protocol for storage commands
    }
    auto result = this->send_receive(COMMAND_STORAGE_GET_METADATA, {(std::byte*)&metaData, sizeof(metaData)});
    if (0 == startProtocol)
    {
        this->setProtocolVersion(startProtocol);
    }
                                                           ...
                                                           uint32_t storageOffset, uint32_t numBytes)
{
    ...
    BOOST_SCOPE_EXIT(startProtocol, this_)
    {
        if (0 == startProtocol)
        {
            this_->setProtocolVersion(startProtocol);
        }
    }
...
bool Sensor::storageWriteStart(TofComm::StorageId_e id, TofComm::StorageMode_e mode)
{
    ...
    uint16_t startProtocol = this->getProtocolVersion();
    if (0 == startProtocol)
    {
        this->setProtocolVersion(1); // cannot use version 0 protocol for storage commands
    }
